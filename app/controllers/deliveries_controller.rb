class DeliveriesController < ApplicationController
  def index
    matching_deliveries = Delivery.where(:user_id => current_user.id, :arrived => false)

    @waiting_deliveries = matching_deliveries.order({ :created_at => :desc })
    @arrived_deliveries = Delivery.where(:user_id => current_user.id, :arrived => true).order({:created_at => :desc})
  ...
  def create
    ...
    the_delivery.user_id = params["query_user_id"]
